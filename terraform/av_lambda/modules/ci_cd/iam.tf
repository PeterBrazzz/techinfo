...
resource "aws_iam_role_policy" "code_build_lambda_updating" {
  name   = local.code_build_iam_lambda_update_policy
  role   = aws_iam_role.code_build.id
  policy = data.aws_iam_policy_document.code_build_lambda_updating.json
}
...
resource "aws_iam_role" "event_bridge_scheduler" {
  name               = substr(local.code_pipeline_scheduler_role, 0, 64)
  tags               = local.code_pipeline_scheduler_tags.iam_role_tags
  assume_role_policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_iam_role_policy" "event_bridge_scheduler" {
  name   = local.code_pipeline_scheduler_policy
  role   = aws_iam_role.event_bridge_scheduler.id
  policy = data.aws_iam_policy_document.code_pipeline_scheduler.json
}
