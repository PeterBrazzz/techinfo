...
variable "code_pipeline_deploy_bucket_arn" {
  type        = string
  description = "CodePipeline deploy bucket ARN."
  validation {
    condition     = length(compact([null, "", var.code_pipeline_deploy_bucket_arn])) > 0
    error_message = "CodePipeline deploy bucket ARN must not be empty."
  }
}
