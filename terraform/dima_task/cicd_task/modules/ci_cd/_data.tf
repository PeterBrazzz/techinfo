...
data "aws_iam_policy_document" "code_build_secret" {
  ...
  dynamic "statement" {
    for_each = length(local.code_build.app_environment_ssm_arns) > 0 ? [true] : []
    content {
      actions   = ["ssm:GetParameters"]
      resources = local.code_build.app_environment_ssm_arns
    }
  }

  dynamic "statement" {
    for_each = length(local.code_build.app_environment_secrets_manager_arns) > 0 ? [true] : []
    content {
      actions = [
        "secretsmanager:GetSecretValue",
        "secretsmanager:DescribeSecret"
      ]
      resources = local.code_build.app_environment_secrets_manager_arns
    }
  }

  dynamic "statement" {
    for_each = length(local.code_build.app_environment_kms) > 0 ? [true] : []
    content {
      actions   = ["kms:Decrypt"]
      resources = local.code_build.app_environment_kms
    }
  }
}
...
# data "aws_iam_policy_document" "code_build_marketing_artifact_connection" {
#   statement {
...
data "aws_iam_policy_document" "code_pipeline_deploy" {
  statement {
    actions = [
      "s3:PutObject",
      "s3:PutObjectVersion",
    ...
    resources = ["${var.code_pipeline_deploy_bucket_arn}/*"]
  }
}
